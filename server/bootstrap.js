  ...
  await strapi.db.lifecycles.subscribe(async (event) => {
    if (event.action === "beforeFindMany" || event.action === "beforeFindOne") {
      ...
      let unnecessaryFields =
        strapi.plugin("custom-deep-populate")?.config("unnecessaryFields") ||
        fieldsToRemove;
      let imageFormats =
        strapi.plugin("custom-deep-populate")?.config("imageFormats") || false;
      let imageInline =
        strapi.plugin("custom-deep-populate")?.config("imageInline") || false;
      const fieldsToKeepInImage =
        strapi.plugin("custom-deep-populate")?.config("fieldsToKeepInImage") ||
        fieldsInImage;
      const removeNestedFieldsWithSameName =
        strapi
          .plugin("custom-deep-populate")
          ?.config("removeNestedFieldsWithSameName") || true;
      const defaultDepth =
        strapi.plugin("custom-deep-populate")?.config("defaultDepth") || 10;

      if (populate && populate[0] === "custom") {
        const depth = populate[1] ?? defaultDepth;
        const newUnnecessaryFields =
          populate
            .slice(2, populate.length)
        ...
        unnecessaryFields = unnecessaryFields.filter(
          (field) => !newUnnecessaryFields.includes(field)
        );
        ...
        if (newUnnecessaryFields.includes("imageFormats"))
